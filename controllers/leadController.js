...
exports.createRemoteLead = async (req, res) => {
    try {
        ...
        const savedData = await RemoteleadData.save();
        res.status(201).json(savedData);
    } catch (err) {
        res.status(400).json({ message: err.message });
    }
...
exports.deleteRemoteLeadById = async (req, res) => {
    const id = req.params.id;
    try {
        const deletedLead = await RemoteLeadData.findByIdAndDelete(id);
        if (!deletedLead) {
            return res.status(404).json({ message: 'Lead not found' });
        }
        res.status(200).json({ message: 'Lead deleted successfully' });
...
exports.getRemoteLeadsByPlatform = async (req, res) => {
    ...
    try {
        ...
        // Find leads that contain the specified platforms
        const remoteleads = await RemoteLeadData.find({ platforms: { $in: selectedPlatforms } });
...
exports.getRemoteLeadsByPlatformAndTag = async (req, res) => {
    ...
    try {
        ...
        // Find leads based on the constructed query
        const remoteleads = await RemoteLeadData.find(query);
